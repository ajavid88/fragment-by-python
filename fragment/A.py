...
# تشخیص خودکار مسیر Xray بر اساس سیستم‌عامل
if platform.system() == "Windows":
    XRAY_PATH = "xray.exe"
else:
    # برای لینوکس: ابتدا در مسیر فعلی، سپس در PATH
    ...
    try:
        ...
        if platform.system() == "Windows":
            ...
            xray_process = subprocess.Popen(
                [XRAY_PATH, "-c", TEMP_CONFIG_FILE],
                creationflags=creationflags,
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL
            )
        else:
            # برای لینوکس: بدون creationflags
            xray_process = subprocess.Popen(
                [XRAY_PATH, "-c", TEMP_CONFIG_FILE],
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL
...
def stop_xray():
    ...
    # پاک‌سازی فرآیندهای باقی‌مانده بر اساس سیستم‌عامل
    if platform.system() == "Windows":
        subprocess.run(["taskkill", "/F", "/IM", "xray.exe"], capture_output=True, check=False)
    else:
        # استفاده از pkill که در اکثر توزیع‌های لینوکس موجود است
        subprocess.run(["pkill", "-9", "-f", "xray"], capture_output=True, check=False)
        # اگر pkill نبود، از killall استفاده کن
        subprocess.run(["killall", "-9", "xray"], capture_output=True, check=False)
...
def measure_speed_and_latency(proxy_url: str | None) -> tuple[float | None, float | None]:
    ...
    try:
        ...
        if lat_res.status_code == 200:
            latency = lat_res.elapsed.total_seconds() * 1000
        
        speed_res = requests.get(
        ...
        if duration_seconds > 0:
            speed = (total_size_bytes * 8) / duration_seconds / 1_000_000
    except requests.exceptions.RequestException:
        pass
...
if __name__ == "__main__":
    try:
        ...
        if not os.path.exists(XRAY_PATH):
            print(f"\n❌ FATAL ERROR: Xray binary not found at '{XRAY_PATH}'")
            print("Please run './setup.sh' first to download Xray-core.")
            input("Press Enter to exit...")
            exit(1)
        
        # بررسی دسترسی اجرا در لینوکس
        if platform.system() != "Windows":
            if not os.access(XRAY_PATH, os.X_OK):
                print(f"\n⚠ Warning: '{XRAY_PATH}' is not executable. Trying to fix...")
                try:
                    os.chmod(XRAY_PATH, 0o755)
                    print("✓ Fixed: Execute permission granted.")
                except Exception as e:
                    print(f"❌ Failed to fix permissions: {e}")
                    print("Please run manually: chmod +x xray")
                    input("Press Enter to exit...")
                    exit(1)
        ...
        with open(RESULTS_FILENAME, 'a', newline='', encoding='utf-8') as csv_file:
            ...
            for i, params in enumerate(all_combinations):
                ...
                if baseline_speed:
                    print(f"  -> Baseline: Speed={baseline_speed:.2f} Mbps | Latency={baseline_latency:.2f} ms")
                else:
                    print("  -> Warning: Could not establish a stable baseline connection.")
                ...
                if start_xray(config):
                    ...
                    if is_accessible:
                        ...
                        if proxy_speed and proxy_latency:
                            ...
                            csv_writer.writerow([
                                timestamp, params['fragment_length'], params['fragment_interval'],
                                params['server_name'], params['dns_server_url'], 'Success',
                                f"{proxy_speed:.2f}", f"{proxy_latency:.2f}"
                            ])
                        else:
                            print("  -> Metrics Test: FAILED")
                            csv_writer.writerow([
                                timestamp, params['fragment_length'], params['fragment_interval'],
                                params['server_name'], params['dns_server_url'], 'Success (Metrics Failed)',
                                'N/A', 'N/A'
                            ])
                    else:
                        ...
                        csv_writer.writerow([
                            timestamp, params['fragment_length'], params['fragment_interval'],
                            params['server_name'], params['dns_server_url'], 'Failed', 'N/A', 'N/A'
                        ])
                    
                    # توقف Xray بعد از هر تست
                    stop_xray()
                    time.sleep(1)
